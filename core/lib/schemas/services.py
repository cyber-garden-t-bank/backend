...
class ServiceCreateView(BaseModel):
    system_name: str = Field(alias="systemName", description="Имя услуги")
    organization_name: str = Field(alias="organizationName", description="Имя организации")
    description: str = Field(description="Описание услуги")
    verbose_name: str = Field(alias="verboseName", description="Имя(красивое для фронта) услуги")
