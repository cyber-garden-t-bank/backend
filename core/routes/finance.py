...
@router.get("/list")
async def get_transactions( token: Annotated[str, Depends(oauth2_scheme)], from_date: Optional[str] = None, to_date: Optional[str] = None, db: AsyncSession = Depends(get_db)) -> list[IncomeFinanceView]:
    ...
    return await selected_list(expr, IncomeTransaction, IncomeFinanceView, db)


@router.post("/create")
async def create_transaction(token: Annotated[str, Depends(oauth2_scheme)],trans: FinanceCreateView, db: AsyncSession = Depends(get_db)) -> IncomeFinanceView:
    token_data = await decode_access_token(token=token, db=db)
    trans.transaction_user = token_data[SUB]
    print(trans)
    return await create_view(IncomeTransaction, trans, IncomeFinanceView, db)


@router.get("/detail/{finance_uuid}")
async def get_transaction(finance_uuid: str, db: AsyncSession = Depends(get_db)) -> IncomeFinanceView:
    expr = (IncomeTransaction.transaction_uuid == finance_uuid)
    return await get_view(IncomeTransaction, IncomeFinanceView, db, by_expr=expr)
